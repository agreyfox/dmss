...
func name2path(projectName string) (string, error) {
	...
	if path == "." {
		...
		fmt.Printf("New project is %s", path)
	}
...
func createProjectInDir(path string) error {
	...
	local := filepath.Join(gopath, "/dms/") //, filepath.Join(repo...))
	...
	if err != nil {
		fmt.Println("Couldn't clone from", local, "- trying again...")
	}
	/* if err != nil {
	...
	staticSrc := filepath.Join("cmd", "dms", "system", "admin", "static")
	err := emptyDir(staticPath)
	if err != nil {
		return err
	}
	fmt.Printf("==>copy the static dir  %s from \n", staticPath, staticSrc)
	err = copyFilesWarnConflicts(staticSrc, staticPath, []string{""})
	if err != nil {
		return err
	}
